...
## 2026-07-04 - Dynamic Checklist Injections
**Learning:** On static pages with many repeating elements, dynamically injecting interactive components (like checkboxes) via JavaScript is more line-efficient than manual HTML editing, helping to stay within strict code change limits. Using the CSS `:has()` selector for state-based visual feedback (like strikethroughs) keeps the implementation clean and avoids manual class toggling.
**Action:** Prefer JS-driven DOM enhancement for repeating patterns to minimize line count and use `:has()` for concise state styling.
